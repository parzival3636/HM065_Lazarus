...
const DeveloperDashboard = () => {
  ...
  const fetchDashboardData = async (userId) => {
    try {
      ...
      const figmaResponse = await fetch('http://127.0.0.1:8000/api/projects/figma/my-shortlists/', {
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      })
      
      if (figmaResponse.ok) {
        const figmaData = await figmaResponse.json()
        const pendingShortlists = (figmaData.shortlists || []).filter(s => !s.figma_submitted)
        setFigmaShortlists(pendingShortlists.slice(0, 2))
      }
    } catch (error) {
  ...
  return (
    <div className="dashboard-container">
      <Navbar user={user} />
      {user?.email && <RejectionNotifications developerEmail={user.email} />}
      <div className="dashboard-wrapper">
        <div className="dashboard-stats">
          ...
          <div className="stat-card">
            ...
            <span className="stat-number">₹{stats.totalEarnings.toLocaleString('en-IN')}</span>
          </div>
          <div className="stat-card">
            <h3>Success Rate</h3>
        ...
        <div className="dashboard-content">
          <div className="main-content">
            {/* Figma Submissions Alert */}
            {figmaShortlists.length > 0 && (
              <section className="figma-alert">
                <div className="alert-banner">
                  <h2>🎨 Figma Submissions Required!</h2>
                  <p>You have {figmaShortlists.length} project(s) waiting for Figma design submission</p>
                  <Link to="/dashboard/developer/figma-submissions" className="btn btn-primary">
          ...
          <aside className="sidebar">
            <div className="quick-actions">
              ...
              <Link to="/dashboard/developer/figma-submissions" className="btn btn-secondary">📐 Figma Submissions</Link>
      ...
      {/* Chatbot */}
      <ProjectChatbot userType="developer" />
    </div>
  )
}
