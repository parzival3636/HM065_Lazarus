...
const CompanyDashboard = () => {
  ...
  const [figmaShortlists, setFigmaShortlists] = useState([])
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch Figma shortlists
      await fetchFigmaShortlists(projects)
    } catch (error) {
      console.error('Failed to fetch dashboard data:', error)
    }
  }

  const fetchFigmaShortlists = async (projects) => {
    try {
      const session = JSON.parse(localStorage.getItem('session') || '{}')
      const shortlistProjects = []

      console.log('🔍 Fetching Figma shortlists for', projects.length, 'projects')
      ...
      for (const project of projects) {
        try {
          ...
          const response = await fetch(
            `http://127.0.0.1:8000/api/projects/${project.id}/figma/get-shortlist/`,
            {
              headers: { 'Authorization': `Bearer ${session.access_token}` }
            }
          )
          ...
          if (response.ok) {
            ...
            if (data.shortlisted && data.shortlist.length > 0) {
              console.log(`  ✅ Found ${data.shortlist.length} shortlisted developers`)
              shortlistProjects.push({
                project_id: project.id,
                project_title: project.title,
                shortlist: data.shortlist,
                submitted_count: data.shortlist.filter(s => s.figma_submitted).length,
                total_count: data.shortlist.length,
                evaluated: data.shortlist.some(s => s.clip_score !== null)
              })
            } else {
              console.log(`  ℹ️  No shortlist for this project`)
            }
          } else {
      ...
      console.log('📊 Total shortlist projects found:', shortlistProjects.length)
      setFigmaShortlists(shortlistProjects)
      
      // Update stats
      setStats(prev => ({
        ...prev,
        figmaSubmissions: shortlistProjects.length
      }))
    } catch (error) {
      console.error('❌ Failed to fetch Figma shortlists:', error)
    }
  }
  ...
  return (
    <div>
      ...
      <div className="dashboard-container">
        <div className="dashboard-stats">
          ...
          <div className="stat-card stat-card-highlight">
            ...
            <span className="stat-number">{stats.figmaSubmissions}</span>
            {stats.figmaSubmissions > 0 && (
              <Link to="/dashboard/company/figma-submissions" className="stat-link">
                View All →
              </Link>
            )}
          </div>
        ...
        <div className="dashboard-content">
          <div className="main-content">
            ...
            <section className="figma-submissions">
              <h2>🎨 Figma Design Submissions</h2>
              {figmaShortlists.length > 0 ? (
                <div className="figma-list">
                  {figmaShortlists.map(item => (
                    <div key={item.project_id} className="figma-item">
                      <div>
                        <h4>{item.project_title}</h4>
                        <div className="figma-stats">
                          <span className="submission-count">
                            {item.submitted_count}/{item.total_count} submitted
                          </span>
                          {item.evaluated && (
                            <span className="evaluated-badge">✓ AI Evaluated</span>
                          )}
                        </div>
                      </div>
                      <Link 
                        to={`/dashboard/company/figma-review/${item.project_id}`}
                ...
                <div className="figma-empty">
                  <p>No design submissions yet. Shortlist top 3 applicants to request Figma designs.</p>
                  <Link to="/dashboard/company/my-projects" className="btn btn-small">
                    View Projects
                  </Link>
                </div>
              )}
          ...
          <aside className="sidebar">
            <div className="quick-actions">
              ...
              <Link 
                to="/dashboard/company/figma-submissions" 
                className={`btn btn-secondary ${figmaShortlists.length > 0 ? 'btn-highlight' : ''}`}
              >
                🎨 Design Submissions {figmaShortlists.length > 0 && `(${figmaShortlists.length})`}
              </Link>
      ...
      {/* Chatbot */}
      <ProjectChatbot userType="company" />
    </div>
  )
}
