...
import { getUserProfile, getDeveloperPortfolio, createPortfolioProject, deletePortfolioProject, incrementPortfolioViews } from '../services/api'
  ...
  const fetchUserAndPortfolio = async () => {
    try {
      ...
      if (userResult.user) {
        ...
        const portfolioResult = await getDeveloperPortfolio(userResult.user.id)
        if (portfolioResult.projects) {
          setProjects(portfolioResult.projects)
        }
      }
    } catch (error) {
