...
export const createCompanyTeam = async (data) => {
  const session = JSON.parse(localStorage.getItem('session') || '{}')
  const response = await fetch(`${API_BASE_URL}/auth/teams/create/`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${session.access_token}`,
      'Content-Type': 'application/json',
...
export const getCompanyTeams = async () => {
  const session = JSON.parse(localStorage.getItem('session') || '{}')
  const response = await fetch(`${API_BASE_URL}/auth/teams/`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${session.access_token}`,
      'Content-Type': 'application/json',
...
export const addToTeam = async (data) => {
  const session = JSON.parse(localStorage.getItem('session') || '{}')
  const response = await fetch(`${API_BASE_URL}/auth/teams/add-member/`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${session.access_token}`,
      'Content-Type': 'application/json',
...
export const getTeamSuggestions = async (data) => {
  const response = await fetch(`${API_BASE_URL}/auth/team-suggestions/`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
